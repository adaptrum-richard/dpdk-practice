...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(10, 8, 3, 42);
...
static int ng_encode_udp_pkt(uint8_t *msg, unsigned char *data, uint16_t total_len)
{
    ...
    struct rte_udp_hdr *udp = (struct rte_udp_hdr *)(msg + sizeof(struct rte_ether_hdr) 
...
static struct rte_mbuf *ng_send_arp(struct rte_mempool *mbuf_pool, uint8_t *dst_mac, 
...
static int ng_encode_icmp_pkt(uint8_t *msg, uint8_t *src_mac, uint8_t *dst_mac, uint32_t sip, uint32_t dip,
...
static struct rte_mbuf *ng_send_icmp(struct rte_mempool *mbuf_pool, uint8_t *src_mac, 
	uint8_t *dst_mac, uint32_t sip, uint32_t dip, uint16_t id, uint16_t seqnb)
{
    ...
    uint8_t *pkt_data = rte_pktmbuf_mtod(mbuf, uint8_t *);
    ng_encode_icmp_pkt(pkt_data, src_mac, dst_mac, sip, dip, id, seqnb);
	return mbuf;
}
#endif
...
{
#if ENABLE_ARP
	char mac_s[RTE_ETHER_ADDR_FMT_SIZE];
	char mac_d[RTE_ETHER_ADDR_FMT_SIZE];
...
#if ENABLE_ARP
	in.s_addr = gLocalIp;
	rte_ether_format_addr(mac_s, RTE_ETHER_ADDR_FMT_SIZE, (struct rte_ether_addr *)gSrcMac);
	inet_ntop(AF_INET, &in, ip_str, INET_ADDRSTRLEN);
	printf("our mac addr: %s, ipaddr: %s\n", mac_s, ip_str);
#endif
    while (1) {
        ...
        for (i = 0; i < num_recvd; i++) {
            ...
            if (ehdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) {
				...
				rte_ether_format_addr(mac_s, RTE_ETHER_ADDR_FMT_SIZE, &ahdr->arp_data.arp_sha);
				rte_ether_format_addr(mac_d, RTE_ETHER_ADDR_FMT_SIZE, &ahdr->arp_data.arp_tha);
				//printf("protocol type:0x%x, OP: 0x%x, sender_mac_addr: %s, target_mac_addr:%s\n", 
				//	ntohs(ahdr->arp_protocol), ntohs(ahdr->arp_opcode), mac_s, mac_d);
			...
			if(iphdr->next_proto_id == IPPROTO_ICMP){
				...
				if(icmphdr->icmp_type == RTE_IP_ICMP_ECHO_REQUEST){
					struct rte_mbuf *txbuf = ng_send_icmp(mbuf_pool, gSrcMac, ehdr->s_addr.addr_bytes,
