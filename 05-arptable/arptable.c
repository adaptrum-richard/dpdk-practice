...
#include <rte_timer.h>
#include <stdio.h>
#define ENABLE_ARP_DEBUG 1
#define ENABLE_ARP_TIMER 1
...
#define TIMER_RESOLUTION_CYCLES 100000000000ULL // 10ms * 1000 = 10s * 6
...
static int ng_encode_arp_pkt(uint8_t *msg, uint16_t opcode, uint8_t *dst_mac, uint32_t sip, uint32_t dip)
{
    ...
    if (!strncmp((const char *)dst_mac, (const char *)gDefaultArpMac, RTE_ETHER_ADDR_LEN)) {
        uint8_t mac[RTE_ETHER_ADDR_LEN] = {0x0}; // arp广播
...
static struct rte_mbuf *ng_send_arp(struct rte_mempool *mbuf_pool, uint16_t opcode, uint8_t *dst_mac,
                                 ...
                                 void *arg)
{
    ...
    int i = 0;
    printf("run arp timer callback func\n");
    for (i = 2; i <= 254; i++) {
